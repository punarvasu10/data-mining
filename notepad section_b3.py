...
def normalize(s):
    s = s.lower(); s = re.sub(r"\d", "#", s); s = re.sub(r"[^a-z#\s]", " ", s)
...
for use_index in [True, False]:
    ...
    for i, row in notices.iterrows():
        for b in range(BANDS):
            cur.execute("INSERT INTO lsh_bands VALUES (?,?,?)", (b, band_hash(sigs[i], b), row["notice_id"]))
    con.commit()
    cur.execute("EXPLAIN QUERY PLAN SELECT notice_id FROM lsh_bands WHERE band_id=0 AND band_hash=12345")
    plan = cur.fetchall()
    t0 = time.time()
    for i in range(1000):
        cur.execute("SELECT notice_id FROM lsh_bands WHERE band_id=0 AND band_hash=?", (band_hash(sigs[i], 0),))
        cur.fetchall()
    dt = time.time() - t0
    print("  use_index=" + str(use_index) + "  plan=" + str(plan) + "  time=" + str(round(dt,3)) + "s")
    con.close()

# =========== (e) DISTRIBUTION + MITIGATION ===========
# build LSH buckets in memory for speed
print("")
print("[e] building in-memory LSH bucket map...")
bucket_map = {}  # (band_id, band_hash) -> [notice indices]
for i in range(len(notices)):
    for b in range(BANDS):
        key = (b, band_hash(sigs[i], b))
        bucket_map.setdefault(key, []).append(i)

# candidate lists per notice (union across bands, with true MinHash similarity)
print("[e] computing candidate lists with similarity scores...")
candidates = []  # list of lists of (other_index, mh_similarity)
for i in range(len(notices)):
    seen = {}
    for b in range(BANDS):
        key = (b, band_hash(sigs[i], b))
    ...
    cand = sorted(seen.items(), key=lambda kv: -kv[1])
...
def pair_retrieved(a, b, cap):
    ...
    if ia not in [x[0] for x in candidates[ib]] and ib not in [x[0] for x in candidates[ia]]:
        return False
    if cap is None:
        return True
    # cap = top-C per notice. Pair survives if a is in top-C of b, or b in top-C of a
    a_in_b = any(x[0] == ia for x in candidates[ib][:cap])
    b_in_a = any(x[0] == ib for x in candidates[ia][:cap])
    return a_in_b or b_in_a

print("")
print("[e] quality vs cap (real truncation)")
for cap in [None, 100, 30, 10, 5]:
    same_ok = same_tot = diff_ok = diff_tot = 0
    for _, r in labels.iterrows():
        a, b, y = r["notice_id_a"], r["notice_id_b"], r["label"]
        if a not in id_to_idx or b not in id_to_idx: continue
        if y == "same":
            same_tot += 1; same_ok += pair_retrieved(a, b, cap)
        else:
            diff_tot += 1; diff_ok += pair_retrieved(a, b, cap)
...
# total work with/without cap
for cap in [None, 100, 30, 10, 5]:
    tot = sum(min(cap, len(c)) if cap else len(c) for c in candidates)
    print("  total candidate pairs with " + ("no cap" if cap is None else "cap="+str(cap)) + ": " + str(tot))
